...
int main(int argc, char** argv)
{
    ...
    clock_t start_time = clock(), final_time;
	...
	for (set<string>::iterator it = strings.begin(); it != strings.end(); ++it) {
		...
		if (table[0][str.length() - 1].find(start) <= table[0][str.length() - 1].length())
			cout << str << " is Part of language\n";
		else
			cout << str << " is Not part of language\n";
	}
    final_time = clock();

    cout << "Program execution took: " << (float)(final_time - start_time)/CLOCKS_PER_SEC << " seconds." << endl;
	return 0;
}